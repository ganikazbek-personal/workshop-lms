...
export function MarkdownContent({ content }: Props) {
  const components: Components = {
    h2: ({ children, id }) => (
      <h2 id={id} style={headingStyle(2)}>
        {children}
      </h2>
    ),
    h3: ({ children, id }) => (
      <h3 id={id} style={headingStyle(3)}>
        {children}
      </h3>
    ),
    p: ({ children }) => (
    ...
    strong: ({ children }) => (
      <strong style={{ color: "var(--ws-text)", fontWeight: 700 }}>
        {children}
      </strong>
    ),
    a: ({ href, children }) => (
    ...
    li: ({ children }) => (
      <li style={{ marginBottom: "0.375rem", color: "var(--ws-text)" }}>
        {children}
      </li>
    ),
    blockquote: ({ children }) => (
    ...
    code: ({ className, children, ...rest }) => {
      ...
      // Block code — delegate to PromptBlock
      const lang = (className ?? "").replace("language-", "") || "bash";
      const codeText = String(children).replace(/\n$/, "");
      return (
        <PromptBlock
          text={codeText}
          filename={lang === "bash" ? "terminal" : lang || "code"}
        />
      );
    },
    // Unwrap pre — PromptBlock handles its own wrapper
...
export function extractHeadings(
  ...
  return markdown
    ...
    .map((line) => {
      ...
      // Approximate github-slugger behavior
      const id = text
        .toLowerCase()
        .replace(/\s+/g, "-")
        .replace(/[^\p{L}\p{N}-]/gu, "");
